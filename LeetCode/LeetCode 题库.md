...
- [LeetCode ——题解](#leetcode-题解)
    ...
    - [989. 数组形式的整数加法](#989-数组形式的整数加法)
        - [笔记： ArrayList() 与 LinkedList()](#笔记-arraylist-与-linkedlist)
...
### 思路1：双循环
1. 建立临时数组，用于存储每一次处理的数组 前半段；
```
int[] tmp = new int[n/2];
```
2. 进入双循环，处理四种情况：
* 当索引 j 在当前行的前半段时，记录当前值，同时将当前位置赋值为 n-1-j 位置的值，注意翻转；
* 当索引 j 在当前行的一半时，偶数长度，将当前位置赋值为 n-1-j 位置的值，注意翻转；
* 当索引 j 在当前行的一半时，奇数长度，将当前位置赋值为 当前 位置的值，注意翻转；
* 当索引 j 在当前行的后半段时，将当前位置赋值为 n-1-j 位置的值，注意翻转；
3. 返回结果即可。
    ...
    public int[][] flipAndInvertImage(int[][] A) {
        ...
        for(int i=0;i<n;i++){
            for(int j=0;j<n;j++){
                ...
                }else if(j == n/2 && n % 2 == 0){//偶数长度，中间的处理
                    A[i][j] = tmp[n-1-j] == 0 ? 1:0;
                }else if(j == n/2 && n % 2 != 0){//奇数长度，中间的处理
                    A[i][j] = A[i][j] == 0 ? 1:0;
                }else if(j > n/2){
                    A[i][j] = tmp[n-1-j] == 0 ? 1:0;
                }
...
### 思路1：双循环
1. 建立二维数组newMatrix，用于记录结果；
2. 进入双循环，将新数组的 [j][i] 位置赋值为原数组的 [i][j] 位置元素；
3. 返回结果。
